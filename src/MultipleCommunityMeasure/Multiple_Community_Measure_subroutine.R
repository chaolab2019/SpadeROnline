...
# Multiple_Community_Measure=function(X,q=2,boot=200,method=c("absolute"))
# { 
#   type <- "abundance"
...
#   info <- c(info, nboot=boot)
#   
#   
#   Cqn=rbind(Cqn_se_equ(X,q=0,boot),
#             #C1n_equ(method="relative",X,boot),
#             NA,
#             C1n_equ(method="absolute",X,boot), 
#             Cqn_se_equ(X,q=2,boot)[1:4])
...
#   for(i in 1:(N-1)){  
#     for(j in (i+1):N){
#       Cqn_PC[k,] <- Cqn_se_equ(X[,c(i,j)],q,boot,method)
...
#   }
#   
#   z <- list("datatype"=type, "info"=info, "overlap"=Cqn, "pairwise"=Cqn_PC, "similarity_matrix"=C_SM, "method"=method, "q"=q)
#   class(z) <- c("spadeMult")
#   z
# }
Multiple_Community_Measure=function(X,datatype=c("abundance","incidence"),q=2,nboot=200,method="relative")
{ 
  if (nboot <= 1)
  ...
  if(datatype=="abundance"){
    ...
    z <- list("datatype"=datatype,"info"=info, "Empirical_richness"=temp[[1]], "Empirical_relative"=temp[[2]], "Empirical_WtRelative"=temp[[3]],
              "Empirical_absolute"=temp[[4]], "estimated_richness"=temp[[5]], "estimated_relative"=temp[[6]], "estimated_WtRelative"=temp[[7]], "estimated_absolute"=temp[[8]], "pairwise"=Cqn_PC, "similarity_matrix"=C_SM, "method"=method, "q"=q)
  }else{
    type <- "incidence"
    Y <- X
    X <- X[-1,]
...
#######################################################2015.09.30
#Y=read.table("Data6a.txt")
#Z=read.table("456.txt")
#Multiple_Community_Measure(X,boot=200)
#Multiple_Community_Measure(Y,q=1,boot=200,method="relative")
#Multiple_Community_Measure(Y,q=1,boot=200,method="absolute")
#Multiple_Community_Measure(Z,boot=200)
