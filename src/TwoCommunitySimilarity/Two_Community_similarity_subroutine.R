###########################################2016.07.19-(P.L.Lin)
#library(Rcpp)
#cppFunction('double shared_entropy(NumericVector x, NumericVector y,int z,int n1,int n2,double w1)
#{
#  double sum=0;
#  int i,t1,t2,k;
...
#  return(sum);
#  }')
#cppFunction('double shared_entropy_sub(NumericVector X1, NumericVector X2,NumericVector p1, NumericVector p2,int I,int n1,int n2,double w1)
#{
#  double sum1=0.0;
#  double sum2=0.0;
#  double temp1=0.0;
...
#  return(sum1+sum2);     
#  }')
#sum_fun=function(X1,X2,w1)
#{
#  n1=sum(X1)
#  n2=sum(X2)
#  w2=1-w1
#  p2hat=correct_obspi(X2)
#  p1hat=correct_obspi(X1) 
#  I=which(X1*X2>0)
#  p2hat=p2hat[I];p1hat=p1hat[I]
#  #sum1=rep(0,length(I))
#  #sum2=rep(0,length(I))
#  sum3=rep(0,length(I)) 
#  X1=X1[I];X2=X2[I]
#  sum1=shared_entropy_sub( X1, X2,p1hat,p2hat,length(I),n1,n2,w1)
#  for(i in 1:length(I) )
#  {
#    temp=-log(w1*p1hat[i]+w2*p2hat[i])-sum(sapply(1:(n1+n2-2),function(r) (1-w1*p1hat[i]-w2*p2hat[i])^r/r))
#    sum3[i]=(w1*p1hat[i])*max(0,temp)        
#  }
#  out=sum1+sum(sum3)-exp(n1*log(w1)+n2*log(w2)+lgamma(n1+n2)-lgamma(n1)-lgamma(n2+1)  )/(n1+n2-1)*sum( p1hat*(1-p1hat)^(n1-1)* (1-p2hat)^(n2) )
#  return(out)       
#}
...
Coverage <- function(data, datatype, m, n)
{
  ...
  sapply(m, FUN = function(i){
    ifelse(datatype=='incidence_raw', Sub2(i), Sub(i) )
  })		
}
Two_com_correct_obspi=function(X1,X2)
{
  ...
  f21=sum(X2==1)
  f22=sum(X2==2)
  C1=Coverage(X1,"abundance", n1, n1)
  C2=Coverage(X2,"abundance", n2, n2)
...
Two_Horn_equ <- function(X1, X2, datatype="abundance", weight="equal",nboot=50, method="all")
{
  if(datatype=="abundance"){
    ...
    if(nboot == 1) {
      commnunity1 <- t(commnunity1)
      commnunity2 <- t(commnunity2)
    } 
    X1 <- X1[-1]
    X2 <- X2[-1]
  }
...
{ 
  ...
  if(q==2){
    if(method=="equal weight"){
      ...
      se.U=sd(b.UqN, na.rm = T);se.U.mle=sd(b.UqN.mle);se.C=sd(b.CqN, na.rm = T);se.C.mle=sd(b.CqN.mle);
      out1=rbind(c(UqN.mle,se.U.mle,min(1,UqN.mle+1.96*se.U.mle),max(0,UqN.mle-1.96*se.U.mle)),
                 c(UqN,se.U,min(1,UqN+1.96*se.U),max(0,UqN-1.96*se.U)));
      out2=rbind(c(CqN.mle,se.C.mle,min(1,CqN.mle+1.96*se.C.mle),max(0,CqN.mle-1.96*se.C.mle)),
                 c(CqN,se.C,min(1,CqN+1.96*se.C),max(0,CqN-1.96*se.C)));
    ...
    if(method=="unequal weight"){
      if(sum(X != 0&X != 1) != 0) {
        ...
        b.UqN.mle=numeric(nboot);b.CqN.mle=numeric(nboot)
        for(i in 1:nboot){
          if(datatype=="abundance"){
            XX=sapply(1:N,function(k) rmultinom(1, ni[k], p[,k]))
          }else{
        ...
        se.U.mle=sd(b.UqN.mle);se.C.mle=sd(b.CqN.mle);
        out1=matrix(c(UqN.mle,se.U.mle,min(1,UqN.mle+1.96*se.U.mle),max(0,UqN.mle-1.96*se.U.mle)), nrow = 1);
        out2=matrix(c(CqN.mle,se.C.mle,min(1,CqN.mle+1.96*se.C.mle),max(0,CqN.mle-1.96*se.C.mle)), nrow = 1)
      }
  ...
  if(dim(out1)[1] == 1) {
    out1 <- round(matrix(c(out1[,c(1, 2)], out1[, 4], out1[, 3]), nrow = 1), 4)
    out2 <- round(matrix(c(out2[,c(1, 2)], out2[, 4], out2[, 3]), nrow = 1), 4)
    rownames(out1)=c("Emperical")
    rownames(out2)=c("Emperical")
  }else {
    out1 <- round(cbind(out1[,c(1, 2)], out1[, 4], out1[, 3]),4)
    out2 <- round(cbind(out2[,c(1, 2)], out2[, 4], out2[, 3]),4)
...
Jaccard_Sorensen_Abundance_equ=function(datatype = c("abundance", "incidence"),X1,X2,boot)
{
  ...
  if(U_tilde+V_tilde-U_tilde*V_tilde==0) JAu=0
  if(U_tilde+V_tilde-U_tilde*V_tilde!=0) JAu=U_tilde*V_tilde/(U_tilde+V_tilde-U_tilde*V_tilde)
  if(U_hat+V_hat-U_hat*V_hat==0) JAa=0
  if(U_hat+V_hat-U_hat*V_hat!=0) JAa=U_hat*V_hat/(U_hat+V_hat-U_hat*V_hat)
  if((U_tilde+V_tilde)==0) SAu=0
  if((U_tilde+V_tilde)!=0) SAu=2*U_tilde*V_tilde/(U_tilde+V_tilde)
  if((U_hat+V_hat)==0) SAa=0
  if((U_hat+V_hat)!=0) SAa=2*U_hat*V_hat/(U_hat+V_hat)
  ...
  for(h in 1:boot)
  {
    ...
    if(length(I) == 0) boot.Bray_Curtis[h] = 0
    if(length(I) != 0) boot.Bray_Curtis[h] = sum(sapply(I,function(I) 2*min(boot.X1[I],boot.X2[I])))/sum(boot.X1+boot.X2)
    ...
    if(boot.U_hat[h]<=1) boot.U_hat[h]=1
    boot.V_hat[h]=boot.V_tilde+(w-1)/w*f1plus/2/f2plus*sum(boot.X2[I][boot.X1[I]==1])/n2
    if(boot.V_hat[h]<=1) boot.V_hat[h]=1
    
    if(boot.U_tilde+boot.V_tilde-boot.U_tilde*boot.V_tilde==0) boot.JAu[h]=0
    if(boot.U_tilde+boot.V_tilde-boot.U_tilde*boot.V_tilde!=0) boot.JAu[h]=boot.U_tilde*boot.V_tilde/(boot.U_tilde+boot.V_tilde-boot.U_tilde*boot.V_tilde)
    if(boot.U_hat[h]+boot.V_hat[h]-boot.U_hat[h]*boot.V_hat[h]==0) boot.JAa[h]=0
    if(boot.U_hat[h]+boot.V_hat[h]-boot.U_hat[h]*boot.V_hat[h]!=0) boot.JAa[h]=boot.U_hat[h]*boot.V_hat[h]/(boot.U_hat[h]+boot.V_hat[h]-boot.U_hat[h]*boot.V_hat[h])
    if(boot.U_tilde+boot.V_tilde==0) boot.SAu[h]=0
    if(boot.U_tilde+boot.V_tilde!=0) boot.SAu[h]=2*boot.U_tilde*boot.V_tilde/(boot.U_tilde+boot.V_tilde)
    if(boot.U_hat[h]+boot.V_hat[h]==0) boot.SAa[h]=0
    if(boot.U_hat[h]+boot.V_hat[h]!=0) boot.SAa[h]=2*boot.U_hat[h]*boot.V_hat[h]/(boot.U_hat[h]+boot.V_hat[h])
...
######################################################
Two_Community_Similarity=function(X, datatype = c("abundance","incidence"),boot=200)
{ 
  nboot = boot
  nboot <- ifelse(nboot <2, 1, nboot)
  if (nboot == 1)
  ...
  if(datatype=="abundance")
  {
    ...
    plus_CI <-function(x){
      if(x[1] >= 1) x[1] <- 1
      if(x[1] <= 0) x[1] <- 0
      c(round(x,4), max(0,x[1]-1.96*x[2]), min(1,x[1]+1.96*x[2]))
    }
    temp <- list()
...
######################Old#######################
# Jaccard_Sorensen_Abundance_equ_old=function(datatype = c("abundance", "incidence"),X1,X2,boot)
# {
#      if(datatype == "incidence")
#      {
#         w=X1[1];z=X2[1]
...
# }
# 
# Chao1_equ_old=function(x,conf=0.95)
# {
#   z <--qnorm((1 - conf)/2)
#   x=x[x>0]
#   D=sum(x>0)
...
# }
# Chao1_bc_equ_old=function(x,conf=0.95)
# {
#   z <- -qnorm((1 - conf)/2)
#   x=x[x>0]
#   D=sum(x>0)
...
# }
# SpecAbunAce_equ_old<- function(data, k=10, conf=0.95)
# {
...
# }
# SpecAbunAce1_equ_old<- function(data ,k=10, conf=0.95)
# {
#   data <- as.numeric(data)
#   f <- function(i, data){length(data[which(data == i)])}
#   basicAbun <- function(data, k){
...
#   return(round(table,1))
# }
# SpecInciChao2_old <-function(data, k=10, conf=0.95)
# {
#   data <- as.numeric(data)
#   z <- -qnorm((1 - conf)/2)
#   t <- data[1]
...
#   return(table)
# }
# SpecInciChao2bc_old <-function(data, k=10, conf=0.95)
# {
#   data <- as.numeric(data)
#   z <- -qnorm((1 - conf)/2)
#   t <- data[1]
...
#   return(table)
# }
# SpecInciModelh_old <-function(data, k=10, conf=0.95)
# {  
#   data <- as.numeric(data)
#   z <- -qnorm((1 - conf)/2)
#   t <- data[1]
...
#   return(table)
# }
# SpecInciModelh1_old <-function(data, k=10, conf=0.95)
# {
#   data <- as.numeric(data)
#   z <- -qnorm((1 - conf)/2)
#   t <- data[1]
...
#    Prob.hat
# }
# entropy_MEE_equ_old=function(X)
# {
#   x=X
#   x=x[x>0]
#   n=sum(x)
...
#   UE+B
# }
# Two_com_correct_obspi_old=function(X1,X2)
# {
#    n1=sum(X1)
#    n2=sum(X2)
#    f11=sum(X1==1)
...
#    return(a)
# }
# C1n_equ_old=function(method=c("relative","absolute"),X,boot=200)
# {
#   X=as.matrix(X)
#   n=colSums(X)
#   min_n=min(n)
...
# }
# 
# Two_Community_Similarity_old=function(X, datatype = c("abundance","incidence"),assemblage1=1,assemblage2=2,boot=200)
# {
#   if(datatype=="abundance")
#   {
#     type="abundance"
...
# 
# 	temp <- list()
# 	temp[[1]] <- Jaccard_Sorensen_Abundance_equ(datatype,X[,1],X[,2],boot)
# 	temp[[1]] <- matrix(sapply(c(temp[[1]]), function(x) ifelse(x==0,"",x)),10)
# 	temp[[1]] <- rbind(temp[[1]][1:5,],
# 					   #c(round(C1n_equ(method="relative",X[,c(1,2)],boot),4)[1:2],"","","",""),
# 					   NA,
# 					   c(round(C1n_equ(method="absolute",X[,c(1,2)],boot),4)[1:2],"","","",""),
# 					   temp[[1]][6:10,])
# 
...
#     
#     cat('(4) Estimation Results of Some Similarity Indices: \n\n')
#     cat('                                      Estimate       s.e.          U_hat* (se.)     V_hat** (se.)\n')
#     cat('    Incidence-based:\n')
#     cat('    ===============\n')
#     temp <- apply(as.matrix(x$similarity), 2, as.numeric)
...
#   }
#   }
#X=read.table("Data5a.txt")
#Y=read.table("Data5b.txt")

#Two_Community_Similarity(datatype="abundance",X,assemblage1=1,assemblage2=2,boot=200)
#Two_Community_Similarity(datatype="incidence",Y,assemblage1=1,assemblage2=2,boot=200)
