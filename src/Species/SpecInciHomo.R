...
function(data, k, conf){
  ...
  diff <- function(q){
    ...
    n_rare = ifelse( (Q(1, x)==0) & (Q(2, x)==0), n_rare+3 , n_rare)
    if ( q == 1){
      d <- (C_infreq - D_infreq*( - ((n_rare*((t - 1)*Q1 + 2*Q2)*2*Q1*(t - 1)) - 
                                       (t - 1)*Q1^2*((t - 1)*(Q1 + n_rare) + 2*Q2))
                                  /(n_rare*((t - 1)*Q1 + 2*Q2))^2)
      )/C_infreq^2
    } else if (q == 2){
      d <- (C_infreq - D_infreq*( - ( - (t - 1)*Q1^2*((t - 1)*Q1*2 + 2*(2*Q2 + n_rare)) 
      )/(n_rare*((t - 1)*Q1 + 2*Q2))^2)
      )/C_infreq^2 
    } else if (q > k){
      d <- 1  
    } else {
      d <- (C_infreq - D_infreq*( - ( - (t - 1)*Q1^2*((t - 1)*Q1*q + 2*Q2*q) 
      )/(n_rare*((t - 1)*Q1 + 2*Q2))^2)
      )/C_infreq^2 
    }
    return(d)
