...
#' @param x a vector of species sample frequencies (for abundance data) or incidence-based sample frequencies (1st entry must be the number of sampling unit).
#' @param q a numeric or a vector of diversity order.
#' @param B an integer to specify the number of replications in the bootstrap procedure.
#' @param datatype a character of data type,"abundance" or "incidence".
...
#------------------------
#' ChaoHill(dat, datatype, from, to, interval, B, conf) is the function of calculating the empirical and the proposed diversity profile, their bootsrap standard errors and confidance intervals.
...
#' @param B an integer to specify the number of bootstrap replications.
#' @param conf a confidence coefficient between 0 and 1.
...
ChaoHill <- function(dat, datatype=c("abundance", "incidence"), q=NULL, from=0, to=3, interval=0.1, B=1000, conf=0.95){ 
  ...
  if(B <= 1) {
    LCI <- UCI <- se <- matrix(NA, 2, length(q))
...
conf.reg=function(x_axis,LCL,UCL,...) {
  if(is.na(sum(LCL))) LCL = UCL <- rep(0, length(x_axis))
  x.sort <- order(x_axis)
  x <- x_axis[x.sort]
  LCL <- LCL[x.sort]
  UCL <- UCL[x.sort]
...
# Reshape ChaoHill output object (list) to data.frame
#' @param out a ChaoHill output object 
reshapeChaoHill <- function(out){
  require(reshape2)
  tab <- data.frame(melt(t(out$EST), value.name = "est", varnames = c("q", "method")),
                    se = melt(t(out$SD),  value.name = "se", varnames = c("q", "method"))[,3],
                    qD.95.LCL = melt(t(out$LCI), value.name = "LCI", varnames = c("q", "method"))[,3],
                    qD.95.UCL = melt(t(out$UCI), value.name = "UCI", varnames = c("q", "method"))[,3])
  tab$q <-as.numeric(substring(tab$q, 5))
  tab
}
